/// TERMP_* environment variable override application.
///
/// Naming convention: TERMP_{SECTION}_{FIELD} in uppercase.
/// Env vars override config file values but not CLI flags (tier 3 of 4).
const std = @import("std");
const Config = @import("Config.zig").Config;

/// Apply TERMP_* environment variable overrides to config.
...
pub fn applyOverrides(config: Config) Config {
    ...
    if (getEnvFloat("TERMP_FONT_SIZE")) |s| result.font.size = s;
    // Note: TERMP_WINDOW_TITLE requires allocated memory; skip for now
    // as env var strings have process lifetime but we'd need to allocate for []const u8.
    if (getEnvI64("TERMP_WINDOW_COLS")) |c| result.window.cols = c;
    if (getEnvI64("TERMP_WINDOW_ROWS")) |r| result.window.rows = r;
    if (getEnvI64("TERMP_SCROLLBACK_LINES")) |l| result.scrollback.lines = l;
    if (getEnvFloat("TERMP_WINDOW_OPACITY")) |o| result.window.opacity = o;
